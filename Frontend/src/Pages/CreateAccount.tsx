...
const CreateAccount: React.FC = () => {
  ...
  return (
    <div className="min-h-screen overflow-y-auto flex items-center justify-center bg-mainbg dark:bg-black w-full">
      <form
        className="flex flex-col lg:flex-row gap-4 w-4/5 p-8 bg-muted/50 border rounded-lg shadow-xl"
            ...
            {formStep === 1 && (
              <>
                ...
                <label className="relative w-full">
                  <textarea
                    className={`bg-muted/50  text-textmain dark:text-white w-full p-4 pt-5 rounded-lg border ${
                      errors.about ? "border-red-600" : "border-gray-600"
                    } outline-none placeholder-transparent focus:placeholder-transparent`}
                    name="about"
                  ...
                  />
                  <span
                    className={`absolute left-3 top-1 transition-all duration-300 ${
                      formData.about
                        ? "text-xs -top-3 text-cyan-500"
                        : "text-sm top-3 text-blue-400"
                    } pointer-events-none`}
                  >
                    About
                  </span>
                ...
                <div className="flex gap-2 mt-4">
                  <button
                    className="w-full py-3 rounded-lg text-white bg-gray-600 hover:bg-gray-700 transition-all"
                    onClick={handleSkip}
                  >
                    Skip
                  </button>
                  <button
                    className="w-full py-3 rounded-lg text-white bg-cyan-500 hover:bg-cyan-600 transition-all"
                    onClick={handleNext}
                  >
                    Save & Next
                  </button>
            ...
            {formStep === 2 && (
              <>
                ...
                <label className="relative w-full">
                  <input
                    className={`bg-muted/50  text-textmain dark:text-white  w-full p-4 pt-5 rounded-lg border ${
                      errors.leetcode ? "border-red-600" : "border-gray-600"
                    } outline-none placeholder-transparent focus:placeholder-transparent`}
                    type="url"
                    name="leetcode"
                ...
                <label className="relative w-full">
                  <input
                    className={`bg-muted/50  text-textmain dark:text-white  w-full p-4 pt-5 rounded-lg border ${
                      errors.gitHub ? "border-red-600" : "border-gray-600"
                    } outline-none placeholder-transparent focus:placeholder-transparent`}
                    type="url"
                    name="gitHub"
                    placeholder="GitHub"
          ...
          <div className="join">
            <input
              className={`join-item btn btn-square ${
                formStep === 1 ? "bg-cyan-500" : "bg-gray-600"
              }`}
              type="radio"
              name="options"
              aria-label="1"
            ...
            <input
              className={`join-item btn btn-square ${
                formStep === 2 ? "bg-cyan-500" : "bg-gray-600"
              }`}
              type="radio"
              name="options"
              aria-label="2"
